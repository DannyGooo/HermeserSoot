...
public class Scene {
  ...
  protected HermesScene hermesScene;
  ...
  public Scene(Singletons.Global g) {
    ...
    } else if (Options.v().src_prec() == Options.src_prec_hbc) {
      addSootBasicHbcClasses();
    } else{
      addSootBasicClasses();
      determineExcludedPackages();
      if(Options.v().react_native()){
        addSootBasicHbcClasses();
      }
    }
  }
  ...
  public void setMainClass(SootClass m) {
    ...
    if (!m.declaresMethod(getSubSigNumberer().findOrAdd("void main(java.lang.String[])"))
        && !m.declaresMethod(getSubSigNumberer().findOrAdd(DotnetMethod.MAIN_METHOD_SIGNATURE))
          && !m.declaresMethod(getSubSigNumberer().findOrAdd(HbcMethod.MAIN_METHOD_SIGNATURE))) {
  ...
  public SootMethod getMainMethod() {
    ...
    SootMethod mainMethod = Options.v().src_prec() == Options.src_prec_dotnet ?
          (mainClass.getMethodUnsafe("Main", Collections.singletonList(ArrayType.v(RefType.v(DotnetBasicTypes.SYSTEM_STRING), 1)), VoidType.v()))
          : 
          ( Options.v().src_prec() == Options.src_prec_hbc ?
            (mainClass.getMethodUnsafe("global", Collections.singletonList(ArrayType.v(RefType.v(HbcTypeFactory.JAVASCRIPT_OBJECT), 1)), RefType.v(HbcTypeFactory.JAVASCRIPT_FUNCTION_OUTPUT)))
            :
            (mainClass.getMethodUnsafe("main", Collections.singletonList(ArrayType.v(RefType.v("java.lang.String"), 1)),VoidType.v())));
  ...
  public RefType getObjectType() {
    ...
    if (Options.v().src_prec() == Options.src_prec_hbc) {
      return getRefType(HbcTypeFactory.JAVASCRIPT_OBJECT);
    }
  ...
  public RefType getBaseExceptionType() {
    ...
    if (Options.v().src_prec() == Options.src_prec_hbc) {
      return getRefType(HbcTypeFactory.JAVASCRIPT_EXCEPTION);
    }
  ...
  public  HermesScene getHermesScene(){
    HermesScene temp = this.hermesScene;
    if (temp == null) {
      temp = new HermesScene();
      this.hermesScene = temp;
    }
    return temp;
  }
  ...
  private void addSootBasicHbcClasses() {
    if(basicclasses[SootClass.HIERARCHY] == null){
      basicclasses[SootClass.HIERARCHY] = new HashSet<>();
    }

    if(basicclasses[SootClass.SIGNATURES] == null){
      basicclasses[SootClass.SIGNATURES] = new HashSet<>();
    }

    if(basicclasses[SootClass.BODIES] == null){
      basicclasses[SootClass.BODIES] = new HashSet<>();
    }

    addBasicClass("java.lang.String");
    addBasicClass(HbcTypeFactory.JAVASCRIPT_HERMES, SootClass.BODIES);

    addBasicClass(HbcTypeFactory.JAVASCRIPT_FUNCTION_OUTPUT, SootClass.SIGNATURES);
    
    addBasicClass(HbcTypeFactory.JAVASCRIPT_THIS, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_FUNCTION, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.HBC_GLOBAL_OBJECT, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_ENVIRONMENT, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_UNKNOWN, SootClass.SIGNATURES);


    addBasicClass(HbcTypeFactory.JAVASCRIPT_ARRAY, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_OBJECT, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_VOID, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_BOOLEAN, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_NUMBER, SootClass.SIGNATURES);

    addBasicClass(HbcTypeFactory.JAVASCRIPT_STRING, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_PHI, SootClass.SIGNATURES);
    
    addBasicClass(HbcTypeFactory.JAVASCRIPT_EXCEPTION, SootClass.SIGNATURES);

    


    addBasicClass(HbcTypeFactory.JAVASCRIPT_SYSTEMEXCEPTION, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_ARITHMETICEXCEPTION, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_ARRAYTYPEMISMATCHEXCEPTION, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_INVALIDCASTEXCEPTION, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_EXCEPTION, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_INDEXOUTOFRANGEEXCEPTION, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_INDEXOUTOFRANGEEXCEPTION, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_OVERFLOWEXCEPTION, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_NULLREFERENCEEXCEPTION, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_NULLREFERENCEEXCEPTION, SootClass.SIGNATURES);
    addBasicClass(HbcTypeFactory.JAVASCRIPT_OUTOFMEMORYEXCEPTION, SootClass.SIGNATURES);
      ...
      if (optsMain != null && !optsMain.isEmpty()) {
        ...
        final List<Type> mainArgs = Collections.singletonList(
            ArrayType.v(Options.v().src_prec() == Options.src_prec_dotnet ? (RefType.v(DotnetBasicTypes.SYSTEM_STRING))
                : (Options.v().src_prec() == Options.src_prec_hbc ? (RefType.v(HbcTypeFactory.JAVASCRIPT_OBJECT)) 
          ...
          boolean declaresMethod
              = Options.v().src_prec() != Options.src_prec_dotnet ? 
                  (Options.v().src_prec() != Options.src_prec_hbc ?
                    ...
                    : c.declaresMethod("global", mainArgs, RefType.v(HbcTypeFactory.JAVASCRIPT_FUNCTION_OUTPUT)))
                  : c.declaresMethod("Main", mainArgs, VoidType.v());
          if (declaresMethod) {
