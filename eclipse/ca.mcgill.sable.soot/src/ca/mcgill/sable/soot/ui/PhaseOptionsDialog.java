...
public class PhaseOptionsDialog extends AbstractOptionsDialog implements SelectionListener {
	...
	private boolean createNewConfig() {
		...
		boolRes = getGeneral_Optionsreact_native_widget().getButton().getSelection();
		defBoolRes = false;

		if (boolRes != defBoolRes) {
			getConfig().put(getGeneral_Optionsreact_native_widget().getAlias(), new Boolean(boolRes));
		}
		boolRes = getGeneral_Optionshbc_control_flow_sensitive_widget().getButton().getSelection();
		defBoolRes = false;

		if (boolRes != defBoolRes) {
			getConfig().put(getGeneral_Optionshbc_control_flow_sensitive_widget().getAlias(), new Boolean(boolRes));
		}
		...
		boolRes = getInput_Optionsresolve_all_hbc_methods_widget().getButton().getSelection();
		defBoolRes = true;

		if (boolRes != defBoolRes) {
			getConfig().put(getInput_Optionsresolve_all_hbc_methods_widget().getAlias(), new Boolean(boolRes));
		}
		...
		stringRes = getInput_Optionshbc_nativehost_path_widget().getText().getText();
		defStringRes = "";

		if ((!(stringRes.equals(defStringRes))) && (stringRes != null) && (stringRes.length() != 0)) {
			getConfig().put(getInput_Optionshbc_nativehost_path_widget().getAlias(), stringRes);
		}
	...
	protected SootOption getInitialInput() {
		...
		//Phase Options
			...
			SootOption wjpp_wjpp_cimbt_branch = new SootOption("Constant reflective method invocation base transformer", "wjppwjpp_cimbt");
			subParent.addChild(wjpp_wjpp_cimbt_branch);


			

			
			subSectParent = wjpp_wjpp_cimbt_branch;
	...
	private BooleanOptionWidget General_Optionsreact_native_widget;
	
	private void setGeneral_Optionsreact_native_widget(BooleanOptionWidget widget) {
		General_Optionsreact_native_widget = widget;
	}
	
	public BooleanOptionWidget getGeneral_Optionsreact_native_widget() {
		return General_Optionsreact_native_widget;
	}	
	
	private BooleanOptionWidget General_Optionshbc_control_flow_sensitive_widget;
	
	private void setGeneral_Optionshbc_control_flow_sensitive_widget(BooleanOptionWidget widget) {
		General_Optionshbc_control_flow_sensitive_widget = widget;
	}
	
	public BooleanOptionWidget getGeneral_Optionshbc_control_flow_sensitive_widget() {
		return General_Optionshbc_control_flow_sensitive_widget;
	}	
	...
	private BooleanOptionWidget Input_Optionsresolve_all_hbc_methods_widget;
	
	private void setInput_Optionsresolve_all_hbc_methods_widget(BooleanOptionWidget widget) {
		Input_Optionsresolve_all_hbc_methods_widget = widget;
	}
	
	public BooleanOptionWidget getInput_Optionsresolve_all_hbc_methods_widget() {
		return Input_Optionsresolve_all_hbc_methods_widget;
	}	
	...
	private StringOptionWidget Input_Optionshbc_nativehost_path_widget;
	
	private void setInput_Optionshbc_nativehost_path_widget(StringOptionWidget widget) {
		Input_Optionshbc_nativehost_path_widget = widget;
	}
	
	public StringOptionWidget getInput_Optionshbc_nativehost_path_widget() {
		return Input_Optionshbc_nativehost_path_widget;
	}
	...
	private BooleanOptionWidget wjppwjpp_cimbtenabled_widget;
	
	private void setwjppwjpp_cimbtenabled_widget(BooleanOptionWidget widget) {
		wjppwjpp_cimbtenabled_widget = widget;
	}
	
	public BooleanOptionWidget getwjppwjpp_cimbtenabled_widget() {
		return wjppwjpp_cimbtenabled_widget;
	}	
	
	private BooleanOptionWidget wjppwjpp_cimbtverbose_widget;
	
	private void setwjppwjpp_cimbtverbose_widget(BooleanOptionWidget widget) {
		wjppwjpp_cimbtverbose_widget = widget;
	}
	
	public BooleanOptionWidget getwjppwjpp_cimbtverbose_widget() {
		return wjppwjpp_cimbtverbose_widget;
	}	
	...
	private Composite General_OptionsCreate(Composite parent) {
		...
		setGeneral_Optionsreact_native_widget(new BooleanOptionWidget(editGroupGeneral_Options, SWT.NONE, new OptionData("React-Native Mode", "", "","rn react-native", "\nRun in react-native program mode, taking into consideration the \njs/hermes code.", defaultBool)));

		defKey = ""+" "+""+" "+"hbc-cf-sensitive hbc-control-flow-sensitive";
		defKey = defKey.trim();
		...
		setGeneral_Optionshbc_control_flow_sensitive_widget(new BooleanOptionWidget(editGroupGeneral_Options, SWT.NONE, new OptionData("hbc-cf-sensitive Mode", "", "","hbc-cf-sensitive hbc-control-flow-sensitive", "\nRun in hbc-control-flow-sensitive program mode, taking into \nconsideration the js/hermes code.", defaultBool)));
	...
	private Composite Input_OptionsCreate(Composite parent) {
		...
		defKey = ""+" "+""+" "+"resolve-all-hbc-methods";
		defKey = defKey.trim();
		...
		setInput_Optionsresolve_all_hbc_methods_widget(new BooleanOptionWidget(editGroupInput_Options, SWT.NONE, new OptionData("Resolve all HBC Methods", "", "","resolve-all-hbc-methods", "\nResolve all hbc methods, such as unsafe methods or methods with \npointers as parameter.", defaultBool)));
		...
		data = new OptionData [] {
				...
				new OptionData("Dotnet Assembly",
						...
						false),
				new OptionData("HBC file",
						"hbc",
						"\nTry to resolve classes from .hbc files found in the Soot \nclasspath.",
						false),
		};
		...
		setInput_Optionshbc_nativehost_path_widget(new StringOptionWidget(editGroupInput_Options, SWT.NONE, new OptionData("HBC NativeHost Path",  "", "","hbc-nativehost-path", "\nUse hbc-nativehost-path to load the NativeHost library which is \nneeded for interpret .hhc.", defaultString)));
	...
	private Composite Processing_OptionsCreate(Composite parent) {
		...
		data = new OptionData [] {
				...
				new OptionData("Dotnet",
						...
						false),
				new OptionData("Hermes Bytecode",
						"hbc",
						"\nSpecialized throw analysis implementation that covers the \nsemantics of .HBC",
						false),
	...
	private Composite wjppwjpp_cimbtCreate(Composite parent) {
		String defKey;
		String defaultString;
		boolean defaultBool = false;
	    String defaultArray;
       
		Group editGroupwjppwjpp_cimbt = new Group(parent, SWT.NONE);
		GridLayout layout = new GridLayout();
		editGroupwjppwjpp_cimbt.setLayout(layout);
	
	 	editGroupwjppwjpp_cimbt.setText("Constant reflective method invocation base transformer");
